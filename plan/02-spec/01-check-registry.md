...
*Erratum (D-70, M1 gate): the ambiguity arm is **vacuously satisfied** — Metacello itself refuses a baseline declaring a group named like a declared package ("incompatible specs"), so the collision can never reach the resolution order; no code implements the arm (E03 C24 stands as cut).*
