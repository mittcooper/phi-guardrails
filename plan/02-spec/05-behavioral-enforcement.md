...
`Phi-Guardrails-Coding-Behavioral`. All SUnit result-object spellings verified live
...
Behavioral suites have **no artifact section of their own**. The kit derives them from
...
`behavioral/<package name>`, scope `#project` (derived entries come from the project's
own baseline and roles, so no global/project ambiguity arises). Adding a test package to
...
`PGRTestSuiteCheck` (kind `#behavioral`) holds one tests-role package name. `run`:
...
`PGRNoSkippedTestsMetaRule` (kind `#behavioral`), registered in the kit block's
...
Registration name:
`behavioral/PGRNoSkippedTestsMetaRule`. It reads **the same result objects** the suite
...
coding kit's `registrationsFrom:` builds one `PGRSuiteRunCache` — a plain object created
...
can reach them. They go in **`Phi-Guardrails-Fixtures-Behavioral`** (D-22): a member of
...
`Phi-Guardrails-Tests-.*` (the verify command and smalltalkCI's `#testing` pattern), and
...
Fixture pair for the meta-rule (R-37): `Phi-Guardrails-Fixtures-Behavioral` holds a
fixture test class with one `skip:` call and one `expectedFailures` entry —
`PGRNoSkippedTestsMetaRuleTest>>#testFiresOnSkipAndExpectedFailure` (in
`Phi-Guardrails-Tests-Coding-Behavioral`) builds a scratch configuration whose baseline
is **`BaselineOfPGRFixture`** — a tiny `BaselineOf` in the fixtures package declaring an
...
shape: `PGRTestSuiteCheckTest>>#testRedOnFailingSuite` (a fixture class with one failing
test) / `>>#testGreenOnPassingSuite`, plus `>>#testMissingOnEmptyTestsRole` for §5.1.

End-to-end (R-44): the toy's tests-role group loads `Phi-Guardrails-Toy-Tests`, which plants one
...
class `FooTest` exists in a tests-role package (`PGRCheckFixturePairMetaRule`, R-46,
