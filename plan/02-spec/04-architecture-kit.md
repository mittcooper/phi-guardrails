...
`Phi-Coding-Kit-Architecture`. All queries are reflective — the engine asks loaded
...
## 4.1 The `#layerMap` block key (D-07 assigned it here; D-51: inside the coding kit's block)

`#layerMap` — a map-valued **parameter key** of the coding kit's block, with two
mandatory keys and one optional:
...
4. Verdict: red iff ≥1 finding, else green. No *violation* is ever an advisory —
   architecture always blocks (D-03) — but the verdict does carry **one** advisory:
...
Fixture pair (R-37): a three-package mini-fixture in `Phi-Coding-Kit-Tests-Architecture`
with one planted forbidden reference — `PCKLayerMapCheckTest>>#testFiresOnForbiddenReference`
...
(`Toy-UI` reaching directly into `-Toy-Persistence`), which R-43's
...
(`Phi-Coding-Kit`, `-Rules`, `-Architecture`, `-Behavioral` — D-57);
